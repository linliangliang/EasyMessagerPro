...
#include "com_xmpp_client_FormLogin_doDaemon.h"
#include "log.h"
#include "constant.h"
...
void waitfor_self_observer(char* observer_file_path){
	...
	if (lockFileDescriptor == -1){
		LOGE("Watched >>>>one OBSERVER<<<< has been ready before watching...");
		return ;
	}

	void *p_buf = malloc(sizeof(struct inotify_event));
	if (p_buf == NULL){
		LOGE("one malloc failed !!!");
		return;
	}
	...
	if (watchDescriptor < 0){
		free(p_buf);
		free(p_maskStr);
		LOGE("one inotify_add_watch failed !!!");
		return;
	}
	...
	while(1){
		...
		if (4 == ((struct inotify_event *) p_buf)->mask){
			LOGE("Watched >>>>one OBSERVER<<<< has been ready...");
			free(p_maskStr);
			free(p_buf);
			return;
		}
...
int lock_file(char* lock_file_path){
    LOGD("one start try to lock file >> %s <<", lock_file_path);
    int lockFileDescriptor = open(lock_file_path, O_RDONLY);
    LOGD("one start try to lock file 1");
    if (lockFileDescriptor == -1){
        lockFileDescriptor = open(lock_file_path, O_CREAT, S_IRUSR);
    }
    LOGD("one start try to lock file 4");
    int lockRet = flock(lockFileDescriptor, LOCK_EX);
    LOGD("one start try to lock file 5");
    if (lockRet == -1){
        LOGE("one lock file failed >> %s <<", lock_file_path);
        return 0;
    }else{
        LOGD("one lock file success  >> %s <<", lock_file_path);
        return 1;
    }
}

JNIEXPORT void JNICALL Java_com_xmpp_client_FormLogin_lockself(JNIEnv *env, jobject jobj, jstring indicatorSelfPath, jstring indicatorDaemonPath, jstring observerSelfPath, jstring observerDaemonPath){
	if(indicatorSelfPath == NULL || indicatorDaemonPath == NULL || observerSelfPath == NULL || observerDaemonPath == NULL){
		LOGE("one parameters cannot be NULL !");
		return ;
	}
	...
	LOGD("one paths %c, %c , %c , %c", indicator_self_path, indicator_daemon_path, observer_self_path,
	...
	while(try_time < 3 && !(lock_status = lock_file(indicator_self_path))){
		try_time++;
		LOGD("one Persistent lock myself failed and try again as %d times", try_time);
		usleep(10000);
	}
	if(!lock_status){
		LOGE("one Persistent lock myself failed and exit");
		return ;
	}

	java_callback(env, jobj, "startDaemonApp");

	LOGD("one lock self file success");
}

JNIEXPORT void JNICALL Java_com_xmpp_client_FormLogin_watch
(JNIEnv *env, jobject jobj, jstring indicatorSelfPath, jstring indicatorDaemonPath, jstring observerSelfPath, jstring observerDaemonPath){
	if(indicatorSelfPath == NULL || indicatorDaemonPath == NULL || observerSelfPath == NULL || observerDaemonPath == NULL){
		LOGE("one parameters cannot be NULL !");
		return ;
	}
...
//	notify_and_waitfor(observer_self_path, observer_daemon_path);
	...
	int lock_status = lock_file(indicator_daemon_path);
	if(lock_status){
		LOGE("Watch >>>>one DAEMON<<<<< Daed !!");
		remove(observer_self_path);// it`s important ! to prevent from deadlock
		remove(indicator_self_path);
		remove(indicator_daemon_path);
		remove(observer_daemon_path);
		java_callback(env, jobj, DAEMON_CALLBACK_NAME);
	}
	LOGE("Watch >>>>one DAEMON<<<<< Over !!");
}