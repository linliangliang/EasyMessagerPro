...
class PacketReader {
    ...
    ///////////////应答消息//////////////////////////////
    public void answerMessage(Packet message) {
        Message ackPacket = new Message();//创建应答消息
        ackPacket.setBody("ack");
        ackPacket.setPacketID(message.getPacketID());
        connection.sendAckPacket(ackPacket);//发送应答
    }
...
//            	 Log.e("000000","11111");
//                 Log.e("!done",(!done)+"");
//                 Log.e("eventType",(eventType != XmlPullParser.END_DOCUMENT)+"");
//                 Log.e("thread",(thread == readerThread)+"");
                if (eventType == XmlPullParser.START_TAG) {
                    if (parser.getName().equals("message")) {
                    	try
                    	{
                    	Message message = (Message)PacketParserUtils.parseMessage(parser); 
//                    	Log.e("rec-----",message.toXML());
                		if(message.getBody().equals("ack"))//应答包
                		{
//                			Log.e("rec----",message.toXML());
                			XmppReliable.getInstance().processAckMessage(message);
                		}
                		else
                		{
                			answerMessage(message);
//                			Log.v("--XmppReliable.reReceiveChack(message.getPacketID())->", ""+XmppReliable.reReceiveChack(message.getPacketID()));
							if(!ReliableDAO.reReceiveChack(message.getPacketID()))//如果不重复，就进行正常处理
							{
								ReliableDAO.insertReceiveChack(message);
//								Log.e("check ", "1.....");
								if(message.getType()==Message.Type.groupchat)
									message.setType(Message.Type.normal);
//								Log.e("rec2!!!",message.toXML());
...
//            Log.e("222222",e.toString());
        }
        Log.e("PacketReader","done="+done);
        if(!(done || connection.isSocketClosed()))
        {   
            Log.e("PacketReader","333333+done="+done);
	        notifyConnectionError(new Exception("disconnect"));
        }
    }
