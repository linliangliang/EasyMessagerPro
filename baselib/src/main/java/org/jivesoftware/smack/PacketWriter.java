...
class PacketWriter {
     ...
     * to the server.
     */
    void startKeepAliveProcess(BaseXmppManager xmppManager) {
...
//                Packet packet = nextPacket();
                ...
                Log.v("packet_send", packet == null ? "null" : packet.toXML());
            }
            ...
            Log.e("packet", packet == null ? "null" : packet.toXML());
        }
    }
    ...
    private class KeepAliveTask implements Runnable {
        ...
        public KeepAliveTask(int delay, BaseXmppManager xmppManager) {
            this.delay = delay;//30s
                ...
                synchronized (writer) {
                    ...
                    if (System.currentTimeMillis() - lastActive >= delay) {
                        try {
                            synchronized (writer) {
//                                writer.write(" ");
                                writer.write("<iq id=\"tTPMA-49\" type=\"get\"><ping xmlns='urn:xmpp:ping' /></iq>");
                                writer.flush();
                            }
                        } catch (SocketException e) {
                            //发送异常，说明与服务器断开连接，切换至断开连接状态，并启动重连接！
                            Log.e("PacketReader", e.toString());
                            connection.disconnect();

                            xmppManager.getConnection().getPacketReader().notifyConnectionError(e);
