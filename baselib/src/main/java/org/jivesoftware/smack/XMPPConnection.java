...
public class XMPPConnection extends Connection {
     ...
     * 徐兵
     */
    private IXmppReliable xmppReliable = null;
     ...
     */
    public XMPPConnection(ConnectionConfiguration config, IXmppReliable xmpp) {
        super(config);
        xmppReliable = xmpp;
    }
    ...
    public void sendPacket(Packet packet) {
        ...
        //校验
        if (packet.toXML().contains("<message") && packet.toXML().contains("<body")) {
            Log.v("message_send", packet.getPacketID());

            MessageDAO mesdao = new MessageDAO();
            int count = mesdao.queryCount(packet.getPacketID());
            if (count == 0) {
                xmppReliable.addMessage(packet.getPacketID(), (Message) packet);
                xmppReliable.addRecMessage(packet.getPacketID(), (Message) packet);
            } else
                xmppReliable.addMessage(packet.getPacketID(), (Message) packet);
        }
    }

    public void sendAckPacket(Packet packet) {
        sendRetryPacket(packet);
    }

    /**
     * 未收到的消息，尝试重发，但不再次存入数据库
     *
     * @param packet
     */

    public void sendRetryPacket(Packet packet) {
        if (!isConnected()) {
            throw new IllegalStateException("Not connected to server.");
        }
        if (packet == null) {
    ...
    private void connectUsingConfiguration(ConnectionConfiguration config) throws XMPPException {
        ...
        try {
            ...
            String errorMessage = "XMPPError connecting to " + host + ":"
                    + port + ". " + ioe;
            if (ioe.toString().contains("ENETUNREACH")) {

                Utils.showToast("网络未连接");
            } else if (ioe.toString().contains("ETIMEDOUT")) {
...
//            packetWriter.startKeepAliveProcess();
    ...
    //启动心跳，并在心跳断开时启动重连接
    public void startKeepAliveThread(BaseXmppManager xmppManager) throws Exception {
